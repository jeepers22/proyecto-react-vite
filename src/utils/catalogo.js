const catalogo = [
    {
        idGenero: 1,
        genero: "Comedia",
        titulo: "Noche de Juegos",
        anio: 2018,
        resenia: "Un grupo de amigos que se reúne una vez a la semana para jugar cambia su rutina y decide resolver un falso asesinato, pero, a medida que la partida avanza, se dan cuenta de que esta vez lo que está ocurriendo es real",
        portada: "https://res.cloudinary.com/datqk3byv/image/upload/v1670307607/noche-de-juegos_b5gamc.jpg"
    },
    {
        idGenero: 1,
        genero: "Comedia",
        titulo: "TED",
        anio: 2012,
        resenia: "Cuando John Bennett (Mark Wahlberg) era pequeño, deseaba que su osito de peluche Ted fuera un oso de verdad y, por desgracia, su sueño se hizo realidad. Más de veinte años después, Ted sigue con John y saca de quicio a su novia Lori, que empieza a perder la paciencia. Para colmo, a John no parece preocuparle su futuro profesional y se pasa la vida fumando porros con Ted. A pesar de ello, John intenta alcanzar la madurez, pero parece que para conseguirlo le resulta indispensable la ayuda de Ted",
        portada: "https://res.cloudinary.com/datqk3byv/image/upload/v1670306305/ted_mit5vw.jpg"
    },
    {
        idGenero: 2,
        genero: "Drama",
        titulo: "En busca de la felicidad",
        anio: 2006,
        resenia: "Chris Gardner es un vendedor brillante y con talento, pero su empleo no le permite cubrir sus necesidades más básicas. Tanto es así que acaban echándolo, junto a su hijo de cinco años, de su piso de San Francisco y no tienen ningún lugar al que ir. Cuando Gardner consigue hacer unas prácticas en una prestigiosa correduría de bolsa, los dos protagonistas tendrán que afrontar muchas adversidades parar hacer realidad su sueño de una vida mejor. Basada en hechos reales",
        portada: "https://res.cloudinary.com/datqk3byv/image/upload/v1670307607/en-busqueda-de-la-felicidad_hy1u94.jpg"
    },
    {
        idGenero: 2,
        genero: "Drama",
        titulo: "El hombre bicentenario",
        anio: 1999,
        resenia: "En la primera década del nuevo milenio, con avances tecnológicos que engullen la soberanía de la compasión humana, Richard Martin compra un regalo, un nuevo robot NDR-114. El hijo más pequeño de la familia le pone de nombre Andrew. Andrew es adquirido como electrodoméstico casero programado para realizar tareas menores. A medida que Andrew empieza a experimentar emociones y pensamiento creativo, la familia Martin descubre pronto que no tienen un robot común y corriente",
        portada: "https://res.cloudinary.com/datqk3byv/image/upload/v1670307607/el-hombre-bicentenario_suupsm.jpg"
    },
    {
        idGenero: 3,
        genero: "Terror",
        titulo: "IT",
        anio: 2017,
        resenia: "En el verano de 1989, un grupo de niños maltratados se unieron para destruir a un monstruo cambia-formas, que se disfraza de payaso y se aprovecha de los niños de Derry, su pequeño pueblo de Maine",
        portada: "https://res.cloudinary.com/datqk3byv/image/upload/v1670307607/it_ibrtbw.jpg"
    },
    {
        idGenero: 3,
        genero: "Terror",
        titulo: "El amanecer de los muertos",
        anio: 2004,
        resenia: "Una enfermera, un policía, un joven matrimonio, un vendedor y otros sobrevivientes de una plaga mundial que está produciendo zombis agresivos y carnívoros, se refugian en un mega centro comercial",
        portada: "https://res.cloudinary.com/datqk3byv/image/upload/v1670307607/el-amanecer-de-los-muertos_v5eayz.jpg"
    }
]

export default catalogo